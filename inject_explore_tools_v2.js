...
Object.keys(translations).forEach(lang => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    // Completely remove the old section if it exists
    const oldExploreSection = document.getElementById('other-tools');
    if (oldExploreSection) {
      oldExploreSection.remove();
    }
    
    // Create new section from HTML template
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = exploreSectionHtml;
    const newSection = tempDiv.querySelector('#other-tools');
    
    // Insert new section right before the bottom CTA
    const bottomCta = document.querySelector('.bottom-cta');
    if (bottomCta) {
      bottomCta.parentNode.insertBefore(newSection, bottomCta);
    } else {
    ...
    // Get the newly inserted section
    const exploreSection = document.getElementById('other-tools');
    
    // Now translate the new section
    if (exploreSection) {
      ...
      while ((n = walk.nextNode())) {
        ...
        if (dict[txt]) {
          ...
          // Check substring matching for Try it free ➔
      ...
      console.log(`Successfully replaced and translated Explore Tools section in ${lang}/index.html`);
    }
  }
});
